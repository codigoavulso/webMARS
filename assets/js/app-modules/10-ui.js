...
function createWindowManager(refs) {
  ...
  function applyToolDesktopPreferredBounds(entry) {
    ...
    const width = Math.max(minWidth, Math.min(widthLimit, Number(pref.width) || minWidth));
  ...
  function toggleMaximize(windowId) {
    ...
    if (isStackedMode() && isStackedFlowEntry(entry)) {
      ...
      if (entry.stackedMaximized) {
        ...
        entry.stackedHeight = Math.max(minHeight, Math.round(window.innerHeight * 0.72));
        entry.stackedExpandedHeight = entry.stackedHeight;
      }
      entry.element.classList.remove("window-maximized");
      layoutStackedWindows(true);
...
function injectRuntimeStyles() {
  ...
  style.textContent = `
    ...
    @media (max-width: 699px) {
      ...
      .desktop.desktop-stacked .desktop-window .window-controls [data-win-action="max"] {
        display: none;
      }
...
function createHelpSystem(refs, messagesPane, windowManager) {
  ...
  const renderAboutCopy = () => `
    <strong>${escapeHtml(translateText("webMARS 0.3"))}</strong><br><br>
    ${escapeHtml(translateText("webMARS is a JavaScript port of the original MARS MIPS 4.5 simulator."))}<br><br>
    ${escapeHtml(translateText("Credits:"))}<br><br>
    &nbsp;&nbsp;&nbsp;&nbsp;${escapeHtml(translateText("Original Java Version: Developed by Pete Sanderson, Kenneth Vollmar, and contributors (2003-2013)."))}<br><br>
    &nbsp;&nbsp;&nbsp;&nbsp;${escapeHtml(translateText("JavaScript Port: Developed and maintained by Nelson Ferreira (2020-2026)."))}<br><br>
    ${escapeHtml(translateText("Privacy & Ethics:"))}<br>
    ${escapeHtml(translateText("This is a completely free, open-source web application. It features no advertising, no cookies, and no user data collection."))}<br><br>
    ${escapeHtml(translateText("Status:"))}<br>
    ${escapeHtml(translateText("Distributed under the MIT License. Please note this is an ongoing project; some features may still be under development or behave differently from the original version."))}<br><br>
    ${escapeHtml(translateText("Contact:"))}<br>
  ...
  aboutWin.innerHTML = `
    ...
    <div class="window-content about-window-content">
      <div class="about-brand">
        <div class="about-copy" id="about-copy">${renderAboutCopy()}</div>
      </div>
  ...
  const titleCloseButton = win.querySelector('[data-win-action="close"]');
  const aboutCopyNode = aboutWin.querySelector("#about-copy");
  ...
  return {
    ...
    refreshTranslations() {
      ...
      if (aboutCopyNode) aboutCopyNode.innerHTML = renderAboutCopy();
  ...
  const definitions = () => {
    ...
    return {
      ...
      Settings: [
        ...
        "-",
        { label: "Language...", command: "showLanguagePreferences" },
        ...
        { label: "Memory Limits...", command: "showMemoryUsagePreferences" }
      ],
