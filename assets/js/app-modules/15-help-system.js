(() => {
  ...
  globalScope.createMarsJavaStyleHelpSystem = function createMarsJavaStyleHelpSystem(refs, messagesPane, windowManager) {
    ...
    const aboutWin = buildWindowShell(
      ...
      `,
      { left: 210, top: 110, minWidth: 440, minHeight: 300 }
    );
    ...
    const aboutHostNode = aboutWin.querySelector("#mars-help-about-host");
    ...
    function scheduleMipsSplit(layout) {
      if (!(layout instanceof HTMLElement)) return;
    ...
    function bindMipsDivider(layout, divider) {
      ...
      scheduleMipsSplit(layout);
      if (typeof ResizeObserver === "function") {
        const observer = new ResizeObserver(() => scheduleMipsSplit(layout));
    ...
    async function renderGeneratedList(host, page) {
      ...
      const wrapped = (markup) => `<div class="mars-help-panel-fill">${markup}</div>`;
      if (page.listKey === "directives") {
        host.innerHTML = wrapped(renderReferenceList(items, "directives"));
        return;
      }
      host.innerHTML = wrapped(renderReferenceList(items, page.listKey));
    }
    ...
    async function renderAboutWindow() {
      const token = ++state.aboutToken;
      aboutTitleNode.textContent = translate(ABOUT_MARS_TITLE);
      if (!(aboutHostNode instanceof HTMLElement)) return;
      renderLoading(aboutHostNode);
      try {
        const path = await resolveHelpPath(ABOUT_INFO_PAGE_FILE);
        if (token !== state.aboutToken) return;
        ...
        if (token !== state.aboutToken) return;
        renderError(aboutHostNode, "Failed to load this help page.");
        postMarsError("[error] Failed to load selected help file.");
      }
    }
