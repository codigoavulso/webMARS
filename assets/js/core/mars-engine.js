...
export class MarsEngine {
  ...
  reset() {
    ...
    // Browser-side file descriptor emulation for syscalls 13-16.
  ...
  captureState() {
    return {
      ...
      openFiles: new Map(this.openFiles),
      virtualFileSystem: new Map(this.virtualFileSystem),
      nextFileDescriptor: this.nextFileDescriptor | 0,
  ...
  restoreState(state) {
    ...
    this.openFiles = new Map(state.openFiles ?? []);
    this.virtualFileSystem = new Map(state.virtualFileSystem ?? []);
    this.nextFileDescriptor = state.nextFileDescriptor | 0;
  ...
  raiseException(cause, message, badAddress = null) {
    ...
    const handlerAddress = EXCEPTION_HANDLER_ADDRESS >>> 0;
    const hasHandler = this.program.textRows.some((row) => row.address === handlerAddress);
    if (hasHandler) {
  ...
  processDataDirective(statement, lineNumber, state, errors, warnings, unresolvedFixups, sizeOnly = false) {
    ...
    if (directive === ".align") {
      const power = args.length ? parseImmediate(args[0]) : 0;
      if (!Number.isFinite(power)) {
        errors.push({ line: lineNumber, message: "Invalid .align value." });
        return;
      }
      if ((power | 0) <= 0) {
        setAutoAlign(false);
      ...
      setAutoAlign(true);
      const alignment = 1 << Math.min(power | 0, 16);
      alignAddress(alignment);
      return;
    }

    if (directive === ".space") {
      const count = args.length ? parseImmediate(args[0]) : 0;
      if (!Number.isFinite(count) || count < 0) {
        errors.push({ line: lineNumber, message: "Invalid .space value." });
        return;
      }
    ...
    if (directive === ".globl" || directive === ".set") {
      return;
    }
    ...
    if (directive === ".extern") {
      if (args.length >= 2) {
        ...
        if (Number.isFinite(size) && size > 0) {
          setAddr(getAddr() + (size | 0));
          return;
        }
      }
      warnings.push({ line: lineNumber, message: "Directive '.extern' is only partially supported." });
      return;
    }

    if (directive === ".include") {
      warnings.push({ line: lineNumber, message: "Directive '.include' is not available in this browser build." });
      return;
    }
    ...
    warnings.push({ line: lineNumber, message: `Directive '${directive}' is not fully implemented yet.` });
  }
  preprocessSourceLines(sourceCode, warnings, errors) {
    ...
    const emit = (lineObj, depth = 0) => {
      if (depth > 20) {
        errors.push({ line: lineObj.lineNumber, message: "Macro expansion depth exceeded." });
        return;
      }
      ...
      if (macros.has(key)) {
        ...
        def.body.forEach((entry) => {
          ...
          emit({ lineNumber: lineObj.lineNumber, statement: expanded }, depth + 1);
        });
        return;
      }

      output.push({ lineNumber: lineObj.lineNumber, statement: eqvApplied });
    };
    ...
    for (let i = 0; i < lines.length; i += 1) {
      ...
      if (/^\.eqv\b/i.test(statement)) {
        ...
        if (!match) {
          errors.push({ line: lineNumber, message: "Malformed .eqv directive." });
          continue;
        }
      ...
      if (/^\.macro\b/i.test(statement)) {
        ...
        if (!header) {
          errors.push({ line: lineNumber, message: "Malformed .macro header." });
          continue;
        }

        const key = `${header.name}/${header.args.length}`;
        if (macros.has(key)) {
          warnings.push({ line: lineNumber, message: `Duplicate macro '${header.name}/${header.args.length}' ignored.` });
        }
        ...
        while (i + 1 < lines.length) {
          i += 1;
          const innerLine = stripComment(lines[i]).trim();
          if (/^\.end_macro\b/i.test(innerLine)) {
            closed = true;
            break;
          }
          if (innerLine) bodyLines.push({ lineNumber: i + 1, statement: innerLine });
        }

        if (!closed) {
          errors.push({ line: lineNumber, message: `Macro '${header.name}' missing .end_macro.` });
          continue;
        }
  ...
  assemble(sourceCode) {
    this.reset();
    this.program.source = sourceCode;

    const warnings = [];
    const errors = [];
    const lines = this.preprocessSourceLines(sourceCode, warnings, errors);
    ...
    const pass1State = {
      ...
      kernelTextAddress: (this.memoryMap.kernelBase ?? 0x80000000) >>> 0,
      kernelDataAddress: 0x90000000,
      autoAlignData: true,
    ...
    lines.forEach((entry) => {
      ...
      if (/^\./.test(statement)) {
        if (pass1State.segment === "data" || pass1State.segment === "kdata") {
          ...
          warnings.push({ line: entry.lineNumber, message: `Directive '${statement.split(/\s+/)[0]}' is ignored in text segment.` });
        }
        return;
      }
  ...
  step() {
    ...
    if (this.halted) {
      return { ok: true, done: true, message: "Program already halted.", snapshot: this.getSnapshot() };
    }
    ...
    const row = this.program.textRows.find((entry) => entry.address === this.pc);
    ...
    const hasNextInstruction = this.program.textRows.some((entry) => entry.address === this.pc);
  ...
  go(maxSteps = 500) {
    ...
    while (executed < maxSteps) {
      ...
      if (stepResult.stoppedOnBreakpoint || stepResult.done) {
        break;
      }
  ...
  executeSyscall() {
    ...
    const promptInput = (label, fallback = "") => fallback;

    const confirmInput = (message) => true;

    const readMessage = (register = 4) => this.readNullTerminatedString(this.registers[register] >>> 0);
    const readBuffer = (address, length) => {
      let text = "";
      for (let i = 0; i < length; i += 1) text += String.fromCharCode(this.getByte((address + i) >>> 0));
      return text;
    };
    ...
    switch (service) {
      case 1:
        return { message: `${this.registers[4] | 0}` };
      case 2:
        return { message: `${this.getFloat32(12)}` };
      case 3:
        return { message: `${this.getFloat64(12)}` };
      case 4:
        return { message: readMessage(4) };
      case 5: {
        const parsed = Number.parseInt(promptInput("ReadInt syscall", "0") ?? "0", 10);
        this.registers[2] = Number.isFinite(parsed) ? clamp32(parsed) : 0;
        return { message: `read_int -> ${this.registers[2] | 0}` };
      }
      case 6: {
        const parsed = Number.parseFloat(promptInput("ReadFloat syscall", "0") ?? "0");
        this.setFloat32(0, Number.isFinite(parsed) ? parsed : 0);
        return { message: `read_float -> ${this.getFloat32(0)}` };
      }
      case 7: {
        const parsed = Number.parseFloat(promptInput("ReadDouble syscall", "0") ?? "0");
        this.setFloat64(0, Number.isFinite(parsed) ? parsed : 0);
        return { message: `read_double -> ${this.getFloat64(0)}` };
      }
      case 8: {
        ...
        const input = promptInput("ReadString syscall", "") ?? "";
      ...
      case 11:
        return { message: String.fromCharCode(this.registers[4] & 0xff) };
      case 12: {
        const value = promptInput("ReadChar syscall", "") ?? "";
        this.registers[2] = value.length ? (value.charCodeAt(0) & 0xff) : 0;
        return { message: `read_char -> ${this.registers[2]}` };
      }
      case 13: {
        const filename = readMessage(4);
        const flag = this.registers[5] | 0;
        if (![0, 1, 9].includes(flag)) {
          this.registers[2] = -1;
          return { message: `open '${filename}' failed (unsupported flag ${flag}).` };
        }
        const existing = String(this.virtualFileSystem.get(filename) ?? "");
        const file = { name: filename, flag, data: existing, cursor: flag === 9 ? existing.length : 0 };
        if (flag === 1) {
          file.data = "";
          file.cursor = 0;
          this.virtualFileSystem.set(filename, "");
        }
      ...
      case 14: {
        ...
        let payload = "";
        if (fd === 0) {
          payload = (promptInput("Read syscall (fd 0)", "") ?? "").slice(0, length);
        } else {
          ...
          payload = file.data.slice(file.cursor, file.cursor + length);
          file.cursor += payload.length;
        }
        for (let i = 0; i < payload.length; i += 1) this.writeByte((address + i) >>> 0, payload.charCodeAt(i) & 0xff);
      ...
      case 15: {
        ...
        const address = this.registers[5] >>> 0;
        const length = Math.max(0, this.registers[6] | 0);
        const output = readBuffer(address, length);
        if (fd === 1 || fd === 2) {
        ...
        if (file.flag === 9) {
          ...
          file.data = `${file.data.slice(0, start)}${output}${file.data.slice(start + output.length)}`;
          file.cursor = start + output.length;
        }
        this.virtualFileSystem.set(file.name, file.data);
        this.registers[2] = output.length;
        return { message: `write ${output.length} byte(s)` };
      }
      case 16:
        this.openFiles.delete(this.registers[4] | 0);
        return { message: `close fd ${this.registers[4] | 0}` };
      case 17:
      ...
      case 31:
      case 33:
        return { message: `midi syscall ${service} ignored in browser build.` };
      case 32:
        return { message: "sleep syscall acknowledged (non-blocking in web build)." };
      case 34:
        return { message: toHex(this.registers[4] >>> 0) };
      case 35:
        return { message: (this.registers[4] >>> 0).toString(2).padStart(32, "0") };
      case 36:
        return { message: `${this.registers[4] >>> 0}` };
      case 40:
      ...
      case 43:
        ...
        return { message: `rand_float -> ${this.getFloat32(0)}` };
      case 44:
        this.setFloat64(0, nextRandomUnit());
        return { message: `rand_double -> ${this.getFloat64(0)}` };
      case 50:
        this.registers[4] = confirmInput(readMessage(4)) ? 0 : 1;
        return { message: `confirm -> ${this.registers[4]}` };
      case 51: {
        const input = promptInput(readMessage(4), "");
        if (input == null) {
          this.registers[4] = 0;
          this.registers[5] = -2;
      ...
      case 59: {
        ...
        if (service === 56) message += `${this.registers[5] | 0}`;
        else if (service === 57) message += `${this.getFloat32(12)}`;
        else if (service === 58) message += `${this.getFloat64(12)}`;
