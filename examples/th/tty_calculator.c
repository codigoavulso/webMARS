...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    //ละเว้นครึ่งหลังของ CRLF เมื่อบัฟเฟอร์ยังว่างเปล่า
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  //เปิด "TTY อุปกรณ์ + ANSI Terminal" เชื่อมต่อกับ MIPS และเรียกใช้ไฟล์นี้
  //รูปแบบที่ยอมรับ: 3 + 4, 8-2, 6 * 7, 20/5
  //พิมพ์ q เพื่อออกจาก REPL
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  int right_box[1] = {0};

  //รักษาเอาต์พุตเริ่มต้นให้สั้นเพื่อให้พร้อมท์ปรากฏขึ้นอย่างรวดเร็ว
