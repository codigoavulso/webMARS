...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    // Ignora a segunda metade do CRLF quando o buffer ainda esta vazio.
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  // Abre a tool "TTY Device + ANSI Terminal", liga-a ao MIPS e corre este ficheiro.
  // Formatos aceites: 3 + 4, 8-2, 6 * 7, 20 / 5.
  // Escreve q para sair do REPL.
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  int right_box[1] = {0};

  // Mantemos o arranque curto para o prompt aparecer rapidamente.
  tty_puts("Calculadora TTY pronta.");
  tty_newline();
  tty_puts("Usa +, -, *, /   |   q sai");
  tty_newline();
  tty_newline();
  ...
  while (true) {
    ...
    if (op_box[0] == 47 && right_box[0] == 0) {
      tty_puts("Nao e permitido dividir por zero.");
      tty_newline();
      continue;
    }
