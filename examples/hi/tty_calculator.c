...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    //जब बफ़र अभी भी खाली हो तो CRLF के दूसरे भाग को अनदेखा करें।
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  //"TTY डिवाइस + ANSI टर्मिनल खोलें", इसे MIPS से कनेक्ट करें, और इस फ़ाइल को चलाएँ।
  //स्वीकृत प्रारूप: 3 + 4, 8-2, 6*7, 20/5।
  //REPL को छोड़ने के लिए q टाइप करें।
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  int right_box[1] = {0};

  //स्टार्टअप आउटपुट को छोटा रखें ताकि प्रॉम्प्ट जल्दी दिखाई दे।
