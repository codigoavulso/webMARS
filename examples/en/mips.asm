# ============================================================
# MARS-OS v0.1 — Bootstrap + Consola gráfica no Bitmap Display
# Bitmap Display:
#   Unit Width/Height: 1
#   Display: 512 x 256
...
#
# Consola:
#   célula 8x8 px => 64 cols x 32 rows
#   scroll por cópia de linhas (copy-up)
# ============================================================
...
.eqv CELL_ROW_BYTES      16384   # 512*8*4 = 8 linhas de píxeis
...
# --- Framebuffer TEM de ser o primeiro item do .data
framebuffer:
  .space FB_BYTES        # base = 0x10010000 (se for mesmo o 1º no .data)
...
# ------------------------------------------------------------
# Glyphs 8x8 (sparse table): cada registo tem 9 bytes:
#   [char_code][row0]..[row7]
# row bits: bit7 = pixel mais à esquerda.
# ------------------------------------------------------------
.align 1
glyph_table:
...
# ------------------------------------------------------------
# console_init: limpa e reset cursor
# ------------------------------------------------------------
console_init:
  addiu $sp, $sp, -8
  sw    $ra, 4($sp)
...
# ------------------------------------------------------------
# fb_clear: fill framebuffer com bg_color
# ------------------------------------------------------------
# ------------------------------------------------------------
# fb_clear: fill framebuffer com bg_color
# versão otimizada: limpa blocos 8x8 (64 pixels)
# ------------------------------------------------------------
fb_clear:
  addiu $sp,$sp,-8
  sw    $ra,4($sp)
  ...
  li    $t3,FB_PIXELS
  srl   $t3,$t3,6          # /64 (64 pixels por loop)
...
# ------------------------------------------------------------
# console_backspace: recua cursor e apaga célula (desenha ' ')
# ------------------------------------------------------------
console_backspace:
  addiu $sp, $sp, -16
  sw    $ra, 12($sp)
  ...
  # se estamos em (0,0) não faz nada
  bnez  $s1, bs_dec_col
  nop
...
bs_store:
  ...
  # desenhar espaço na célula atual
  li    $a0, 0x20
  move  $a1, $s1
  move  $a2, $s0
  jal   draw_char_cell
...
# ------------------------------------------------------------
# console_newline: col=0; row++; scroll se necessário
# ------------------------------------------------------------
console_newline:
  addiu $sp, $sp, -12
  sw    $ra, 8($sp)
...
# ------------------------------------------------------------
# console_scroll: copia framebuffer 8 linhas para cima + limpa última banda
# ------------------------------------------------------------
console_scroll:
  addiu $sp, $sp, -12
  sw    $ra, 8($sp)
...
# ------------------------------------------------------------
# kernel_main: lê teclado por MMIO e faz echo
# ------------------------------------------------------------
kernel_main:
  li   $t0,KBD_CTRL
