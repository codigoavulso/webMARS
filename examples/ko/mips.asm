#MARS-OS 1.0 - 완전한 대화형 TTY 운영 환경
#도구 > TTY 장치 + ANSI 터미널을 열고 MIPS에 연결합니다.
#조립하고 실행하십시오. 마우스 데스크탑이 열립니다. 셸용 터미널을 시작합니다.
#
#이 파일에는 현지화된 텍스트만 들어 있습니다. 커널 자체는
#하단에 포함된 언어 중립 모듈:
#mips_os_lib.asm TTY 드라이버, ANSI 제어, 문자열, 정수
#mips_os_fs.asm 쓰기 가능 RAM 파일 시스템 및 해당 명령
#mips_os_apps.asm 계산기, 배너, 달력, 게임
#mips_os_edit.asm 전체 화면 텍스트 편집기
#mips_os_sheet.asm 전체 화면 스프레드시트
#mips_os_basic.asm 줄 번호 정수 BASIC 인터프리터
#mips_os_desktop.asm 마우스 구동 프로그램 관리자 및 창
#mips_os_kernel.asm 부팅, 셸 및 시스템 명령
...
#---- 창 바탕화면 ----
#메뉴 표시줄 제목 및 메뉴 항목. 데스크탑은 메뉴를 다음과 같이 배치합니다.
#이러한 문자열을 사용하므로 번역이 길수록 메뉴가 더 넓어집니다.
...
#---- 부팅 시 디스크 이미지 ----
#모든 블록은 개행 문자를 사용합니다. RAM 디스크는 일반 텍스트를 저장하고
#콘솔 드라이버는 나가는 길에 줄바꿈을 CR + LF로 바꾸는 것입니다.
