...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    //버퍼가 아직 비어 있으면 CRLF의 후반부를 무시합니다.
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  //"TTY 장치 + ANSI 터미널"을 열고 MIPS에 연결한 후 이 파일을 실행합니다.
  //허용되는 형식: 3 + 4, 8-2, 6 * 7, 20 / 5.
  //REPL를 종료하려면 q를 입력하세요.
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  int right_box[1] = {0};

  //프롬프트가 빨리 나타날 수 있도록 시작 출력을 짧게 유지하십시오.
