...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    //Ignorez la seconde moitié de CRLF lorsque le tampon est encore vide.
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  //Ouvrez "TTY Device + ANSI Terminal", connectez-le à MIPS et exécutez ce fichier.
  //Formats acceptés : 3+4, 8-2, 6*7, 20/5.
  //Tapez q pour quitter le REPL.
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  int right_box[1] = {0};

  //Gardez la sortie de démarrage courte pour que l’invite apparaisse rapidement.
