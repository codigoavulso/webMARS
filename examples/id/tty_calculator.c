...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    //Abaikan paruh kedua CRLF saat buffer masih kosong.
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  //Buka "TTY Perangkat + ANSI Terminal", sambungkan ke MIPS, dan jalankan file ini.
  //Format yang diterima: 3+4, 8-2, 6*7, 20/5.
  //Ketik q untuk keluar dari REPL.
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  int right_box[1] = {0};

  //Jaga agar output startup tetap singkat sehingga perintah muncul dengan cepat.
