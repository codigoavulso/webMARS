...
void draw_ascii_line(int x, int y, int cols, int rows, int glyphs[], int length, int fg, int bg) {   // cada glifo se dibuja píxel a píxel en el framebuffer
  int index = 0;
  while (index < length) {
    bitmap_ascii_put_char(x + index * 6, y, cols, rows, glyphs[index], fg, bg);   // 6 píxeles por carácter: la fuente no trae espaciado propio
    index = index + 1;
  }
}
...
int main(void) {
  int cols = 128;
  int rows = 64;
  int bg = 0x00000000;   // una palabra por píxel, 0x00RRGGBB
  int fg = 0x00ffffff;
