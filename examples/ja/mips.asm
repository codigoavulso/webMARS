#MARS-OS 1.0 - 完全な対話型 TTY オペレーティング環境
#[ツール] > TTY デバイス + ANSI ターミナルを開き、MIPS に接続します。
#組み立てて実行します。マウスのデスクトップが開きます。シェルのターミナルを起動します。
#
#このファイルにはローカライズされたテキストのみが含まれます。カーネル自体は
#下部に含まれる言語中立モジュール:
#mips_os_lib.asm TTY ドライバー、ANSI コントロール、文字列、整数
#mips_os_fs.asm 書き込み可能 RAM ファイルシステムとそのコマンド
#mips_os_apps.asm 電卓、バナー、カレンダー、ゲーム
#mips_os_edit.asm フルスクリーン テキスト エディター
#mips_os_sheet.asm 全画面スプレッドシート
#mips_os_basic.asm 行番号付き整数 BASIC インタープリター
#mips_os_desktop.asm マウス駆動のプログラム マネージャーとウィンドウ
#mips_os_kernel.asm ブート、シェル、およびシステム コマンド
...
#---- ウィンドウデスクトップ ----
#メニュー バーのタイトルとメニュー エントリ。デスクトップにはメニューがレイアウトされます。
#これらの文字列なので、翻訳を長くしてもメニューが広くなるだけです。
...
#ウィンドウのタイトル。ウィンドウの種類ごとにインデックスが付けられます。
...
#---- 起動時のディスクイメージ ----
#すべてのブロックは裸の改行を使用します。RAM ディスクにはプレーン テキストが保存され、
#コンソールドライバーは、途中で改行を CR と LF に変換します。
