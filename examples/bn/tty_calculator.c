...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    //CRLF এর দ্বিতীয়ার্ধ উপেক্ষা করুন যখন বাফারটি এখনও খালি থাকে।
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  //"TTY ডিভাইস + ANSI টার্মিনাল খুলুন, এটি MIPS] এর সাথে সংযুক্ত করুন এবং এই ফাইলটি চালান।
  //গৃহীত বিন্যাস: 3 + 4, 8-2, 6 * 7, 20 / 5।
  //REPL ছেড়ে যেতে q টাইপ করুন।
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  int right_box[1] = {0};

  //স্টার্টআপ আউটপুট ছোট রাখুন যাতে প্রম্পট দ্রুত প্রদর্শিত হয়।
