...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    //Zignoruj ​​drugą połowę CRLF, gdy bufor jest nadal pusty.
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  //Otwórz „TTY Urządzenie + ANSI Terminal”, podłącz je do MIPS i uruchom ten plik.
  //Akceptowane formaty: 3+4, 8-2, 6*7, 20/5.
  //Wpisz q, aby opuścić REPL.
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  int right_box[1] = {0};

  //Zachowaj krótkie dane wyjściowe uruchamiania, aby monit pojawiał się szybko.
