...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    //تجاهل النصف الثاني من CRLF عندما يكون المخزن المؤقت فارغًا.
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  //افتح "TTY Device + ANSI Terminal"، وقم بتوصيله بـ MIPS، وقم بتشغيل هذا الملف.
  //الصيغ المقبولة: 3+4، 8-2، 6*7، 20/5.
  //اكتب q لترك REPL.
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  ...
  //اجعل مخرجات بدء التشغيل قصيرة حتى تظهر المطالبة بسرعة.
