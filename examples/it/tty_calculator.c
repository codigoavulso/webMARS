...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    //Ignora la seconda metà di CRLF quando il buffer è ancora vuoto.
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  //Apri "TTY Dispositivo + ANSI Terminale", collegalo a MIPS ed esegui questo file.
  //Formati accettati: 3+4, 8-2, 6*7, 20/5.
  //Digitare q per uscire da REPL.
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  int right_box[1] = {0};

  //Mantieni l'output di avvio breve in modo che il prompt venga visualizzato rapidamente.
