...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    //وقتی بافر هنوز خالی است، نیمه دوم CRLF را نادیده بگیرید.
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  //"TTY Device + ANSI ترمینال را باز کنید، آن را به MIPS متصل کنید و این فایل را اجرا کنید.
  //فرمت های پذیرفته شده: 3 + 4، 8-2، 6 * 7، 20 / 5.
  //برای خروج از REPL q را تایپ کنید.
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  int right_box[1] = {0};

  //خروجی راه اندازی را کوتاه نگه دارید تا اعلان سریع ظاهر شود.
