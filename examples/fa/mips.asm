#MARS-OS 1.0 - یک محیط عملیاتی تعاملی کامل TTY
#باز کردن ابزارها > TTY دستگاه + ANSI ترمینال، اتصال به MIPS،
#جمع آوری و اجرا؛ دسکتاپ ماوس را باز می کند. ترمینال را برای پوسته راه اندازی کنید.
#
#این فایل فقط متن محلی را نگه می دارد. خود هسته در آن زندگی می کند
#ماژول های خنثی زبان موجود در پایین:
#mips_os_lib.asm TTY درایور، ANSI کنترل، رشته‌ها، اعداد صحیح
#سیستم فایل mips_os_fs.asm قابل نوشتن RAM و دستورات آن
#ماشین حساب mips_os_apps.asm، بنر، تقویم، بازی ها
#ویرایشگر متن تمام صفحه mips_os_edit.asm
#mips_os_sheet.asm صفحه گسترده تمام صفحه
#مفسر خط mips_os_basic.asm شماره صحیح BASIC
#mips_os_desktop.asm با ماوس مدیریت برنامه و ویندوز
#دستورات بوت، پوسته و سیستم mips_os_kernel.asm
...
#---- دسکتاپ پنجره دار ----
#عناوین نوار منو و ورودی های منو. دسکتاپ منوها را از آن بیرون می گذارد
#این رشته ها، بنابراین یک ترجمه طولانی تر فقط منو را گسترده تر می کند.
...
#عناوین پنجره، نمایه شده بر اساس نوع پنجره.
...
#---- تصویر دیسک زمان بوت ----
#هر بلوک از خطوط جدید استفاده می کند: دیسک RAM متن ساده و
#درایور کنسول چیزی است که یک خط جدید را در هنگام خروج به CR به علاوه LF تبدیل می کند.
