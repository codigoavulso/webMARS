...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    //Игнорируйте вторую половину CRLF, если буфер еще пуст.
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  //Откройте «TTY Устройство + ANSI Терминал», подключите его к MIPS и запустите этот файл.
  //Принимаемые форматы: 3+4, 8-2, 6*7, 20/5.
  //Введите q, чтобы выйти из REPL.
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  int right_box[1] = {0};

  //Старайтесь, чтобы выходные данные при запуске были короткими, чтобы приглашение появлялось быстро.
