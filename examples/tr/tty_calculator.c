...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    //Arabellek hala boşken CRLF işleminin ikinci yarısını dikkate almayın.
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  //"TTY Cihaz + ANSI Terminali"ni açın, onu MIPS'ye bağlayın ve bu dosyayı çalıştırın.
  //Kabul edilen formatlar: 3 + 4, 8-2, 6 * 7, 20 / 5.
  //REPL'den çıkmak için q yazın.
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  int right_box[1] = {0};

  //İstemin hızlı bir şekilde görünmesi için başlangıç çıktısını kısa tutun.
