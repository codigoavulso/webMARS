...
void draw_ascii_line(int x, int y, int cols, int rows, int glyphs[], int length, int fg, int bg) {   //her glif çerçeve arabelleğine piksel piksel çizilir
  int index = 0;
  while (index < length) {
    bitmap_ascii_put_char(x + index * 6, y, cols, rows, glyphs[index], fg, bg);   //Karakter başına 6 piksel: yazı tipinin kendine ait bir boşluğu yoktur
    index = index + 1;
  }
}
...
int main(void) {
  int cols = 128;
  int rows = 64;
  int bg = 0x00000000;   //piksel başına bir kelime, 0x00RRGGBB
  int fg = 0x00ffffff;
