...
void draw_ascii_line(int x, int y, int cols, int rows, int glyphs[], int length, int fg, int bg) {   //Jede Glyphe wird Pixel für Pixel in den Framebuffer gezeichnet
  int index = 0;
  while (index < length) {
    bitmap_ascii_put_char(x + index * 6, y, cols, rows, glyphs[index], fg, bg);   //6 Pixel pro Zeichen: Die Schriftart verfügt über keinen eigenen Abstand
    index = index + 1;
  }
}
...
int main(void) {
  int cols = 128;
  int rows = 64;
  int bg = 0x00000000;   //ein Wort pro Pixel, 0x00RRGGBB
  int fg = 0x00ffffff;
