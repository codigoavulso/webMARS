...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    //Ignorieren Sie die zweite Hälfte von CRLF, wenn der Puffer noch leer ist.
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  //Öffnen Sie „TTY Gerät + ANSI Terminal, verbinden Sie es mit MIPS und führen Sie diese Datei aus.
  //Akzeptierte Formate: 3 + 4, 8-2, 6 * 7, 20 / 5.
  //Geben Sie q ein, um REPL zu verlassen.
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  int right_box[1] = {0};

  //Halten Sie die Startausgabe kurz, damit die Eingabeaufforderung schnell erscheint.
