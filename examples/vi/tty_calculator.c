...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    //Bỏ qua nửa sau của CRLF khi bộ đệm vẫn trống.
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  //Mở "TTY Device + ANSI Terminal", kết nối nó với MIPS và chạy tệp này.
  //Các định dạng được chấp nhận: 3 + 4, 8-2, 6 * 7, 20/5.
  //Gõ q để rời khỏi REPL.
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  int right_box[1] = {0};

  //Giữ đầu ra khởi động ngắn để lời nhắc xuất hiện nhanh chóng.
