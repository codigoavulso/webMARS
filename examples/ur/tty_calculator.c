...
int read_line(int buffer[], int capacity) {
  ...
  while (true) {
    ...
    //جب بفر ابھی بھی خالی ہو تو CRLF کے دوسرے نصف حصے کو نظر انداز کریں۔
    if (ch == 13 || ch == 10) {
      if (len == 0) {
        continue;
      }
...
int main(void) {
  //"TTY ڈیوائس + ANSI ٹرمینل کھولیں، اسے MIPS سے جوڑیں، اور اس فائل کو چلائیں۔
  //قبول شدہ فارمیٹس: 3 + 4، 8-2، 6 * 7، 20 / 5۔
  //REPL کو چھوڑنے کے لیے q ٹائپ کریں۔
  int line[64] = {0};
  int left_box[1] = {0};
  int op_box[1] = {0};
  int right_box[1] = {0};

  //اسٹارٹ اپ آؤٹ پٹ کو مختصر رکھیں تاکہ پرامپٹ تیزی سے ظاہر ہو۔
