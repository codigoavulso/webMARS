...
#该文件仅保存本地化文本。内核本身位于
#底部包含的语言中性模块：
#mips_os_lib.asm TTY 驱动程序、ANSI 控制、字符串、整数
#mips_os_fs.asm 可写 RAM 文件系统及其命令
#mips_os_apps.asm 计算器、横幅、日历、游戏
#mips_os_edit.asm 全屏文本编辑器
#mips_os_sheet.asm 全屏电子表格
#mips_os_basic.asm 行编号整数 BASIC 解释器
#mips_os_desktop.asm 鼠标驱动的程序管理器和窗口
#mips_os_kernel.asm 引导、shell 和系统命令
...
#---- 启动时磁盘映像 ----
#每个块都使用裸换行符：RAM 磁盘存储纯文本，
#控制台驱动程序将换行符转换为 CR 加 LF 。
